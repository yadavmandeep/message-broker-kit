...
export class RedisAdapter implements IMessageBroker {
  ...
  public async connectProducer(): Promise<RedisClientType> {
    if (this.publisher) return this.publisher;
    this.publisher = createClient({ url: this.url, password: this.password }) as RedisClientType;
  ...
  public async connectConsumer(): Promise<RedisClientType> {
    if (this.subscriber) return this.subscriber;
    this.subscriber = createClient({ url: this.url, password: this.password }) as RedisClientType;
