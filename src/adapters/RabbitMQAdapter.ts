...
export class RabbitMQAdapter implements IMessageBroker {
  ...
  public async connectProducer(): Promise<amqp.Channel> {
  ...
  public async connectConsumer(): Promise<amqp.Channel> {
  ...
  public async subscribe(messageHandler: MessageHandler, topic: string): Promise<amqp.Channel> {
    ...
    }); // consume returns `{ consumerTag: string }` but we just return channel
