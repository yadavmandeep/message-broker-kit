...
export class EnterpriseBrokerWrapper implements IMessageBroker {
  private baseBroker: IMessageBroker;
  private options: EnterpriseOptions;
  private idempotencyCache: LRUCache<string, boolean> | null = null;
  ...
  private static instanceRefs: EnterpriseBrokerWrapper[] = [];
  private static shutdownInitialized: boolean = false;
  ...
  constructor(baseBroker: IMessageBroker, options: EnterpriseOptions = {}) {
    ...
    if (this.options.idempotency?.enabled) {
      this.idempotencyCache = new LRUCache({
        max: 10000,
  ...
  public async publish(data: PublishData): Promise<any> {
    ...
    if (this.options.idempotency?.enabled && !headers['messageId']) {
      headers['messageId'] = uuidv4();
    }

    // 4. Payload Encryption
    if (this.options.encryption?.enabled && this.options.encryption.secretKey) {
        data.message = CryptoUtils.processPayload(
            data.message, 
            this.options.encryption.fieldsToEncrypt || [], 
            'encrypt', 
  ...
  public async subscribe(messageHandler: MessageHandler, topic: string): Promise<any> {
    ...
    const wrappedHandler: MessageHandler = async (incoming: IncomingMessage) => {
        ...
        if (this.options.encryption?.enabled && this.options.encryption.secretKey && incoming.headers['x-encrypted'] === 'true') {
           incoming.data = CryptoUtils.processPayload(
               incoming.data, 
               this.options.encryption.fieldsToEncrypt || [], 
               'decrypt', 
               this.options.encryption.secretKey
