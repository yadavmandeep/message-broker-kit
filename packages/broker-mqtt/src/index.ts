...
export class MQTTAdapter implements IMessageBroker {
  ...
  public async connectProducer(): Promise<any> {
    if (!this.producerClient) this.producerClient = await this.connectClient('pub');
    return this.producerClient;
  }

  public async disconnectProducer(): Promise<void> {
    if (this.producerClient) { this.producerClient.end(); this.producerClient = null; }
  }

  public async publish(data: PublishData): Promise<any> {
    const client = await this.connectProducer();
    const payload = JSON.stringify({ headers: data.headers || {}, event: data.event, data: data.message });
    return new Promise((resolve, reject) => {
  ...
  public async connectConsumer(): Promise<any> {
    if (!this.consumerClient) this.consumerClient = await this.connectClient('sub');
    return this.consumerClient;
  }

  public async disconnectConsumer(): Promise<void> {
    if (this.consumerClient) { this.consumerClient.end(); this.consumerClient = null; }
  }
